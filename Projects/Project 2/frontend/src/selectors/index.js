...
const getComments = state =>
  state.comments.filter(post => post.deleted === false);
  ...
  (posts, comments) => {
    return posts.map(post => ({
      ...
      comments: comments.filter(
        comment => comment.deleted === false && comment.parentId === post.id
      )
    }));
  }
);
...
export const getSortedPostsWithSortedComments = createSelector(
  [getSortedPosts, getSortedComments],
  (sortedPosts, sortedComments) => {
    return sortedPosts.map(post => ({
      ...post,
      comments: sortedComments.filter(
        comment => comment.deleted === false && comment.parentId === post.id
      )
    }));
  }
